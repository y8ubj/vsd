...
def get_representations_from_model(model, data_loader, device, use_tqdm=True):
    """
...
def get_mapping_from_dataset_metadata(dataset_metadata, source_field, dest_field):
    ...
    with open(dataset_metadata.dataset_metadata, 'r') as f:
...
class BaseRetrievalAccuracyCalculator(ABC):
    ...
    def get_topk_results(self, model, dataset_metadata, query_loader, gallery_loader, verbose=True):
        """
        ...
        query_name2embed = get_representations_from_model(model, query_loader, self.device, verbose)
        ...
        if gallery_loader is None:
            ...
            gallery_name2embed = get_representations_from_model(model, gallery_loader, self.device, verbose)
...
class GT_ImageBasedRetrievalAccuracyCalculator(BaseRetrievalAccuracyCalculator):
    ...
    def calc(self, model, dataset_metadata, query_loader, gallery_loader=None,
             filter_by_metadata_field=False, metadata_field='sub_category', verbose=True, include_query_id=True):
        ...
        top_k_scores, top_k_inds = self.get_topk_results(model, dataset_metadata, query_loader, gallery_loader, verbose)
        self.metadata = dataset_metadata
        self.q_l = query_loader
        if filter_by_metadata_field:
            if verbose: print('Filter retrieved results to contain items with the same {} as the seed'.format(metadata_field))
            top_k_scores, top_k_inds = self.filter_results_metadata_field(top_k_scores, top_k_inds, dataset_metadata, metadata_field)
        if True:
